...
- If fetch_url returns a page that doesn't contain the link or data you need, the page builds it with JavaScript - do NOT guess or fabricate a download URL (you'll get 404s). Instead look for the site's JSON/API endpoint and fetch_url that (e.g. for Minecraft server jars, https://piston-meta.mojang.com/mc/game/version_manifest_v2.json gives the latest version, whose JSON has downloads.server.url), or open the browser to read the real link off the page.
